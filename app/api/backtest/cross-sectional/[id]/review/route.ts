...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doc = await crossSectionalBacktests.findOne({ _id: objectId, ownerId });
  if (!doc) {
  ...
  const prompt = `You are reviewing a backtested cross-sectional (rank-and-rotate) trading strategy over the NIFTY 50 universe for a personal paper-trading app (simulation only, no real money, no broker). Give a concise, plain-language critique.

Strategy: ${strategy.name} (${strategy.description})
Params: ${JSON.stringify(doc.config.params)}
Universe: NIFTY 50 (all 50 stocks ranked and rebalanced), interval ${doc.config.interval}, range ${doc.config.range}
Rebalance frequency: ${strategy.rebalanceFrequency ?? "monthly"}
...
- Buy-and-hold (equal-weight universe) return over the same period: ${doc.metrics.buyHoldReturnPct.toFixed(2)}%

Trade log (rebalance turnover):
${tradeSummary || "(no trades were made)"}

Comment on whether this ranking approach beat a passive equal-weight holding of the universe, whether the drawdown/Sharpe suggest real risk, signs of overfitting to this specific window or universe, and what you'd change before trusting it further. Keep it under 200 words.`;

  try {
    const result = await chat([{ role: "user", content: prompt }], { task: "backtest_review" });
    const aiReview = { ...result, createdAt: new Date() };
    await crossSectionalBacktests.updateOne({ _id: objectId }, { $set: { aiReview } });
