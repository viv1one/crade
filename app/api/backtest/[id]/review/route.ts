...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doc = await backtests.findOne({ _id: objectId, ownerId });
  if (!doc) {
  ...
  const prompt = `You are reviewing a backtested trading strategy for a personal paper-trading app (simulation only, no real money, no broker). Give a concise, plain-language critique.

Strategy: ${strategy.name} (${strategy.description})
Params: ${JSON.stringify(doc.config.params)}
Symbol: ${doc.config.symbol}, interval ${doc.config.interval}, range ${doc.config.range}
...
Comment on whether this strategy beat buy-and-hold, whether the drawdown/Sharpe suggest real risk, signs of overfitting to this specific window, and what you'd change before trusting it further. Keep it under 200 words.`;

  try {
    const result = await chat([{ role: "user", content: prompt }], { task: "backtest_review" });
    const aiReview = { ...result, createdAt: new Date() };
    await backtests.updateOne({ _id: objectId }, { $set: { aiReview } });
