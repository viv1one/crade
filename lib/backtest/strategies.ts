...
export const STRATEGIES: Record<StrategyId, StrategyDef> = {
  ...
  trend_following: {
    ...
    description:
      "Time-series trend rule: buy while the trailing return over the lookback window is positive, " +
      "sell once it turns negative. Judges the symbol against its " +
  ...
  fifty_two_week_high: {
    ...
    name: "52-Week High",
    description:
      "Buy while price is within nearPct of its trailing 52-week high, sell once it falls more than " +
      "exitPct below that high. Nearness to the high itself " +
      "is the signal here, not the return that got it there.",
    kind: "single_symbol",
    family: "Trend & Momentum",
    paramSchema: [
  ...
  momentum_factor: {
    ...
    description:
      "Ranks the NIFTY 50 universe by trailing return and holds the top topN, equal-weighted, " +
      "rebalanced monthly — ranked against peers, not " +
      "just against its own past (that's Trend Following, a single-symbol strategy).",
  ...
  turn_of_month: {
    ...
    description:
      "Buy during the window from daysBeforeEnd calendar days before month-end through " +
      "daysAfterStart days into the next month, flat the rest of the time — a calendar effect. " +
      "Applied per-symbol here rather than to a broad index — same idea, whatever symbol is loaded.",
    kind: "single_symbol",
  ...
  short_term_reversal: {
    ...
    description:
      "Ranks the NIFTY 50 universe by the worst trailing weekly return and holds the top topN, " +
      "equal-weighted, rebalanced weekly — the " +
  ...
  betting_against_beta: {
    ...
    description:
      ...
      "own equal-weight return and holds the lowest-beta topN. Not the textbook long-short " +
...
type SingleSymbolSignalFn = (
  bars: HistoricalBar[],
  params: StrategyParams,
  auxiliaryBars?: HistoricalBar[]
) => Signal[];

// Lookup-map dispatch, mirroring cross-sectional-strategies.ts's
// SCORE_FUNCTIONS/getScoreFn pattern rather than a growing switch.
// auxiliaryBars is only actually read by strategies whose StrategyDef sets
// `auxiliary` (currently just crude_oil_predictor) — every other entry
// here ignores the third argument.
const SIGNAL_FUNCTIONS: Partial<Record<StrategyId, SingleSymbolSignalFn>> = {
  sma_crossover: smaCrossoverSignals,
  rsi_mean_reversion: rsiMeanReversionSignals,
  momentum_breakout: momentumBreakoutSignals,
  ml_momentum: (bars, params) => generateMlSignals(bars, params),
  trend_following: trendFollowingSignals,
  fifty_two_week_high: fiftyTwoWeekHighSignals,
  turn_of_month: turnOfMonthSignals,
  payday_anomaly: paydayAnomalySignals,
  option_expiry_week: optionExpiryWeekSignals,
  january_barometer: (bars) => januaryBarometerSignals(bars),
  overnight_anomaly: overnightAnomalySignals,
  momentum_reversal_vol: momentumReversalVolSignals,
  crude_oil_predictor: crudeOilPredictorSignals,
};
...
export function generateSignals(
  ...
  const fn = SIGNAL_FUNCTIONS[strategyId];
  if (!fn) throw new Error(`Unknown or non-single-symbol strategy: ${strategyId}`);
  return fn(bars, params, auxiliaryBars);
}
